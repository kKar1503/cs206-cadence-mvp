  ...
  const listings = await Promise.all([
    ...
    prisma.listing.create({
      data: {
        ...
        sleeveScore: 92,
        labelConditionScore: 90,
        edgesScore: 88,
        conditionNotes: "Sleeve and label look clean. No visible warping, scuffs, or ring wear detected.",
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Come Together", "Something", "Maxwell's Silver Hammer", "Oh! Darling", "Octopus's Garden", "I Want You (She's So Heavy)"] },
          { side: "B", tracks: ["Here Comes the Sun", "Because", "You Never Give Me Your Money", "Sun King", "Mean Mr. Mustard", "Polythene Pam", "She Came In Through the Bathroom Window", "Golden Slumbers", "Carry That Weight", "The End", "Her Majesty"] },
        ]),
        sellerId: users[0]!.id,
    ...
    prisma.listing.create({
      data: {
        ...
        sleeveScore: 70,
        labelConditionScore: 78,
        edgesScore: 78,
        conditionNotes: "Cover shows moderate wear and creasing. Vinyl surface has light surface marks but plays well.",
        tracklist: JSON.stringify([
          { side: "A", tracks: ["So What", "Freddie Freeloader", "Blue in Green"] },
          { side: "B", tracks: ["All Blues", "Flamenco Sketches"] },
        ]),
        sellerId: users[0]!.id,
    ...
    prisma.listing.create({
      data: {
        ...
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Second Hand News", "Dreams", "Never Going Back Again", "Don't Stop", "Go Your Own Way", "Songbird"] },
          { side: "B", tracks: ["The Chain", "You Make Loving Fun", "I Don't Want to Know", "Oh Daddy", "Gold Dust Woman"] },
        ]),
        sellerId: users[1]!.id,
    ...
    prisma.listing.create({
      data: {
        ...
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Speak to Me", "Breathe", "On the Run", "Time", "The Great Gig in the Sky"] },
          { side: "B", tracks: ["Money", "Us and Them", "Any Colour You Like", "Brain Damage", "Eclipse"] },
        ]),
        sellerId: users[1]!.id,
    ...
    prisma.listing.create({
      data: {
        ...
        conditionScore: 75,
        vinylSurfaceScore: 78,
        sleeveScore: 70,
        labelConditionScore: 76,
        edgesScore: 76,
        conditionNotes: "Well used with minor shelf wear. Vinyl plays well with some surface noise on a few tracks.",
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Let's Go Crazy", "Take Me with U", "The Beautiful Ones", "Computer Blue", "Darling Nikki"] },
          { side: "B", tracks: ["When Doves Cry", "I Would Die 4 U", "Baby I'm a Star", "Purple Rain"] },
        ]),
        sellerId: users[2]!.id,
    ...
    prisma.listing.create({
      data: {
        ...
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Wanna Be Startin' Somethin'", "Baby Be Mine", "The Girl Is Mine", "Thriller"] },
          { side: "B", tracks: ["Beat It", "Billie Jean", "Human Nature", "P.Y.T. (Pretty Young Thing)", "The Lady in My Life"] },
        ]),
        sellerId: users[2]!.id,
    ...
    prisma.listing.create({
      data: {
        ...
        tracklist: JSON.stringify([
          { side: "A", tracks: ["All I Want", "My Old Man", "Little Green", "Carey", "Blue"] },
          { side: "B", tracks: ["California", "This Flight Tonight", "River", "A Case of You", "The Last Time I Saw Richard"] },
        ]),
        sellerId: users[3]!.id,
        images: JSON.stringify([imageUrls["blue.jpg"]]),
        imageUrl: imageUrls["blue.jpg"],
        priceLabel: "FAIR",
        pricePercentage: 3.1,
    ...
    prisma.listing.create({
      data: {
        ...
        conditionScore: 74,
        vinylSurfaceScore: 77,
        sleeveScore: 70,
        labelConditionScore: 75,
        edgesScore: 74,
        conditionNotes: "Cover shows age-appropriate wear. Vinyl plays well despite surface marks.",
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Sunday Morning", "I'm Waiting for the Man", "Femme Fatale", "Venus in Furs", "Run Run Run", "All Tomorrow's Parties"] },
          { side: "B", tracks: ["Heroin", "There She Goes Again", "I'll Be Your Mirror", "The Black Angel's Death Song", "European Son"] },
        ]),
        sellerId: users[3]!.id,
        images: JSON.stringify([imageUrls["the-velvet-underground.jpg"]]),
        imageUrl: imageUrls["the-velvet-underground.jpg"],
        priceLabel: "OVERPRICED",
    ...
    prisma.listing.create({
      data: {
        ...
        conditionScore: 76,
        vinylSurfaceScore: 74,
        sleeveScore: 77,
        labelConditionScore: 77,
        edgesScore: 76,
        conditionNotes: "Light scratches visible but plays without skipping. Lyric sheet included.",
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Smells Like Teen Spirit", "In Bloom", "Come as You Are", "Breed", "Lithium", "Polly"] },
          { side: "B", tracks: ["Territorial Pissings", "Drain You", "Lounge Act", "Stay Away", "On a Plain", "Something in the Way"] },
        ]),
        sellerId: users[3]!.id,
        images: JSON.stringify([imageUrls["nevermind.jpg"]]),
        imageUrl: imageUrls["nevermind.jpg"],
        priceLabel: "UNDERPRICED",
        pricePercentage: 8.6,
    ...
    prisma.listing.create({
      data: {
        ...
        tracklist: JSON.stringify([
          { side: "A", tracks: ["What's Going On", "What's Happening Brother", "Flyin' High (In the Friendly Sky)", "Save the Children", "God Is Love", "Mercy Mercy Me (The Ecology)"] },
          { side: "B", tracks: ["Right On", "Wholy Holy", "Inner City Blues (Make Me Wanna Holler)"] },
        ]),
        sellerId: users[4]!.id,
        images: JSON.stringify([imageUrls["whats-going-on.jpg"]]),
        imageUrl: imageUrls["whats-going-on.jpg"],
        priceLabel: "FAIR",
        pricePercentage: 1.9,
    ...
    prisma.listing.create({
      data: {
        ...
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Airbag", "Paranoid Android", "Subterranean Homesick Alien"] },
          { side: "B", tracks: ["Exit Music (For a Film)", "Let Down", "Karma Police"] },
          { side: "C", tracks: ["Fitter Happier", "Electioneering", "Climbing Up the Walls", "No Surprises"] },
          { side: "D", tracks: ["Lucky", "The Tourist"] },
        ]),
        sellerId: users[4]!.id,
        images: JSON.stringify([imageUrls["ok-computer.jpg"]]),
        imageUrl: imageUrls["ok-computer.jpg"],
        priceLabel: "UNDERPRICED",
    ...
    prisma.listing.create({
      data: {
        ...
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Give Life Back to Music", "The Game of Love", "Giorgio by Moroder"] },
          { side: "B", tracks: ["Within", "Instant Crush", "Lose Yourself to Dance"] },
          { side: "C", tracks: ["Touch", "Get Lucky", "Beyond"] },
          { side: "D", tracks: ["Motherboard", "Fragments of Time", "Doin' It Right", "Contact"] },
        ]),
        sellerId: users[4]!.id,
        images: JSON.stringify([imageUrls["random-access-memories.jpg"]]),
        imageUrl: imageUrls["random-access-memories.jpg"],
        priceLabel: "OVERPRICED",
    ...
    prisma.listing.create({
      data: {
        ...
        isVerified: false,
        tracklist: JSON.stringify([
          { side: "A", tracks: ["Rainy Day Women #12 & 35", "Pledging My Time", "Visions of Johanna", "One of Us Must Know (Sooner or Later)"] },
          { side: "B", tracks: ["I Want You", "Stuck Inside of Mobile with the Memphis Blues Again", "Leopard-Skin Pill-Box Hat", "Just Like a Woman"] },
          { side: "C", tracks: ["Most Likely You Go Your Way and I'll Go Mine", "Temporary Like Achilles", "Absolutely Sweet Marie", "4th Time Around", "Obviously 5 Believers"] },
          { side: "D", tracks: ["Sad Eyed Lady of the Lowlands"] },
        ]),
        sellerId: users[4]!.id,
        images: JSON.stringify([imageUrls["blonde-on-blonde.jpg"]]),
        imageUrl: imageUrls["blonde-on-blonde.jpg"],
        priceLabel: "UNDERPRICED",
        pricePercentage: 9.3,
    ...
    prisma.listing.create({
      data: {
        ...
        isVerified: false,
        sellerId: users[2]!.id,
        images: JSON.stringify([imageUrls["beatles-vintage-band-shirt.jpg"]]),
        imageUrl: imageUrls["beatles-vintage-band-shirt.jpg"],
        priceLabel: "OVERPRICED",
